...
def form_post(request):
    ...
    if request.method == "POST":
        ...
        dict_obj = request.POST.dict()
        p = list(request.POST.copy().values())
        
        instance=get_object_or_404(Detail,Document_Name=dict_obj['Document_Name'])
        field=Detail._meta.get_fields()
        field=[f.name for f in field ]
        # print(field)
        field.remove('doc')
        ...
        try:
            role_query=Role.objects.filter(Role_Name=dict_obj['Roles'])
            role_id=list(role_query.values('id'))
            role_id=role_id[0]['id']
        except:
            r=Role(Role_Name=dict_obj['Roles'])
            r.save()
            role_query=Role.objects.filter(Role_Name=dict_obj['Roles'])
            role_id=list(role_query.values('id'))
            role_id=role_id[0]['id']
        ...
        if form.is_valid():
            print("Form save")
            p=form.save()
            p.Role_ref_id=role_query=Role.objects.get(Role_Name=dict_obj['Roles']).id
            p.save()
            return Response(status=200)
